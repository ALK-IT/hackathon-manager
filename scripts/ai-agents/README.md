...
```powershell
gh secret set DISCORD_WEBHOOK_URL --repo ALK-IT/hackathon-manager
```
...
```powershell
./scripts/ai-agents/watch-repo.ps1 -Repo ALK-IT/hackathon-manager -Once
```

Ciągłe odpytywanie (proces działa w tle, co 5 min):

```powershell
./scripts/ai-agents/watch-repo.ps1 -Repo ALK-IT/hackathon-manager -PollSeconds 300
```

Ręczne odpalenie na konkretnym PR:

```powershell
./scripts/ai-agents/review-pr.ps1 -Repo ALK-IT/hackathon-manager -PRNumber 12
```

## Harmonogram (Windows Task Scheduler)

```powershell
schtasks /create /tn "ai-review-hackathon-manager" /sc minute /mo 10 `
  /tr "powershell.exe -File `"$PWD\scripts\ai-agents\watch-repo.ps1`" -Repo ALK-IT/hackathon-manager -Once" `
  /f
```
