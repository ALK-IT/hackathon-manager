...
async def test_list_active_without_filters_returns_all_non_deleted_hackathons(
    ...
    result, total = await repository.list_active()
...
async def test_list_active_filters_upcoming_hackathons(session: AsyncSession):
    ...
    assert await repository.list_active(upcoming=True) == ([upcoming], 1)
    assert await repository.list_active(upcoming=False) == ([started], 1)
...
async def test_list_active_filters_effective_registration_state(session: AsyncSession):
    ...
    assert await repository.list_active(registration_open=True) == ([opened], 1)
    assert await repository.list_active(registration_open=False) == (
        [manually_closed, expired, scheduled],
        3,
    )


async def test_list_active_applies_limit_and_offset_and_keeps_total(session: AsyncSession):
    owner = await persist_user(session, email="owner@example.com")
    hackathons = [
        make_hackathon(owner, name=f"Hackathon {index}", created_offset=index) for index in range(3)
    ]
    session.add_all(hackathons)
    await session.commit()
    repository = HackathonRepository(session)

    result, total = await repository.list_active(limit=1, offset=1)
...
async def test_list_managed_returns_only_owned_and_co_organized_hackathons(
    ...
    result, total = await repository.list_managed_by_user(current_user.id)

    assert result == [co_organized, owned]
    assert total == 2


async def test_list_managed_applies_limit_and_offset_and_keeps_total(session: AsyncSession):
    current_user = await persist_user(session, email="current@example.com")
    hackathons = [
        make_hackathon(current_user, name=f"Managed {index}", created_offset=index)
    ...
    result, total = await repository.list_managed_by_user(current_user.id, limit=1, offset=1)
